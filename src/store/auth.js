...
export default {
  ...
  actions: {
    async getRegisterData({ commit, dispatch }, payload) {
      const APIkey = "AIzaSyDP_N3G2mdf0be8Wmu4NcyKe5An6dQjT40";
      const authUrl =
  "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=";
    ...
    async addNewUser({ commit, state }, payload) {
      try {
          await axios.put(
              `https://final-test-vue-53ca3-default-rtdb.firebaseio.com/user/${payload.userId}.json?auth=${state.token}`,
              payload
          );
    ...
    async getUser({ commit }, payload) {
      try {
        const { data } = await axios.get (
          `https://final-test-vue-53ca3-default-rtdb.firebaseio.com/user.json`)
          for (let key in data) {
    ...
    async updateUserProfile({ state }, updatedData) {
      try {
        ...
        await axios.patch(
          `https://final-test-vue-53ca3-default-rtdb.firebaseio.com/user/${userId}.json?auth=${state.token}`,
          updatedData
        );
